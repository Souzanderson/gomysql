...
func Get[T interface{}](table string, options *GetProps) ([]T, error) {
	...
	if db == nil {
		_, err := connect()
		if err != nil {
			fmt.Println("[ERROR] Error on connect mysql bank!")
			return nil, err
		}
	}
	...
	if err != nil {
		fmt.Println("[ERROR] Error on Query!" + err.Error())
		return nil, err
	}

	columns, err := rows.Columns()
	if err != nil {
		fmt.Println("[ERROR] Error on Select Columns!")
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Println("[ERROR] Error on Get Row!")
			return nil, err
		}
...
func Set[T interface{}](table string, value T) (int64, error) {
	...
	if err != nil {
		fmt.Println("Erro ao Preparar Insert => ", err.Error())
		return 0, err
	}
	...
	if err != nil {
		fmt.Println("Erro ao Executar Insert => ", err.Error())
		return 0, err
	}

	id, err := response.LastInsertId()
	if err != nil {
		fmt.Println("Erro ao Contar Insert => ", err.Error())
		return 0, err
	}
	// fmt.Println(id)
	return id, nil
...
func Update[T interface{}](table string, value T, where string) (int64, error) {
	...
	if err != nil {
		fmt.Println("Erro ao Preparar Update => ", err.Error())
		return 0, err
	}
	...
	if err != nil {
		fmt.Println("Erro ao Executar Update => ", err.Error())
		return 0, err
	}

	id, err := response.RowsAffected()
	if err != nil {
		fmt.Println("Erro ao Contar Update => ", err.Error())
		return 0, err
	}
	// fmt.Println(id)
	return id, nil
